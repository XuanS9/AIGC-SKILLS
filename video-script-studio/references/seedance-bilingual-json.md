...
> Seedance skill.** The canonical skill is `higgsfield-seedance.md`
> (the `EN-director` profile); the shared engine-rule core that all profiles —
> `EN-director`, `ZH-house`, `bilingual-JSON` — obey lives in
> `higgsfield-seedance-engine-rules.md`. The § ENGINE RULES section below
> is retained so this persona stays self-contained when pasted into an external
> runtime, but on rule conflicts `higgsfield-seedance-engine-rules.md` wins. Use this profile only
...
### 1. Double contrast (conditional technique)
When the event benefits from strong contrast, change **both** shot size **and** camera character. Repeated static dialogue coverage, match cuts and deliberate repeated compositions remain valid. Do not add cuts or override approved camera direction to satisfy a pattern.
...
### 3. Inserts: any scale, causally motivated
Inserts direct attention to a detail. A 0.3–0.5s static punctuation is one option; information reveals or prop-state changes need enough time to read. Any shot size.

**Rules:**
- Identify the insert's function: punctuation, information reveal or state change. It becomes a separate narrative stage only when the progression requires one.
...
- Use double contrast (§1) when motivated; preserve intentional continuity.
...
- Choose cut contrast by narrative purpose; retain intentional repeated coverage
