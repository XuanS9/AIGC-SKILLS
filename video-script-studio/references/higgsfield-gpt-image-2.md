...
When an image becomes a video reference, hand its identity, material, composition and permitted changes to [shared director planning](narrative-planning.md). Panels are asset views, not automatically video stages or shots; identify each panel's reference role. Standalone images retain the JSON, prose and meta-prompt formats below without temporal fields.
...
```json
{
  ...
  "style": "clean museum interface, soft pastel palette, generous whitespace",
  "layout": {
    "header": {
      "logo": "small black wordmark 'GLASS ARCHIVE'",
      "nav": ["Collection", "Materials", "Journal", "About"],
      "search_icon": "top-right"
    },
    "hero": {
      "left_side": "large product photo of amber glass serum bottle on marble",
      "right_side": {
        "headline": "Amber glass, close up.",
        "subheadline": "Study the shape, finish, and light transmission.",
        "detail_button": "black pill button 'View object'"
      }
    },
    "below_hero": {
      "detail_grid": {"count": 4, "labels": ["Silhouette", "Material", "Surface", "Construction"]}
    }
  }
}
```
...
For image tasks, deliver the requested prompt directly; keep the brief routing line required by [the platform guide](higgsfield-guide.md).
...
This is the surface-specific operationalization of the cross-cutting pre-delivery discipline. See [DISCIPLINE.md](production-checks.md) § Pre-Delivery Discipline for the underlying pattern (the v3.7.5 + v3.7.7 audit-corpus discipline applied across multiple sub-skills).
...
Use [`higgsfield-gpt-image-2-reference-sheet-workflow.md`](higgsfield-gpt-image-2-reference-sheet-workflow.md) to turn a product or prop image into a multi-view identity reference. For character turnarounds, continue with [Soul](higgsfield-soul.md); for video, assign each reference a role in [shared director planning](narrative-planning.md).
