...
Apply [shared director planning](narrative-planning.md) before adapting a recipe: identify the actual goal, events, visible responses and final state, then allocate stages, shots and legal generation durations. Recipe arcs, shot counts and example timings are selectable starting points; preserve the user's fixed content and do not fill missing slots merely to match a recipe.
...
> categories in `negative-constraints.md`. Action recipes → Body/Motion; Horror →
...
- `template-*.md` — Annotated prompt templates expand on these recipes with line-by-line breakdowns
