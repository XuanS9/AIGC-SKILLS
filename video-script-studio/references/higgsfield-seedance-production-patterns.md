...
model parameter. Read alongside `higgsfield-seedance.md` § Official Prompt Architecture —
...
Model-parameter facts referenced here come from `model-specs.json`
...
These phrases map onto `higgsfield-seedance.md` § Reference Roles: `100% matches` is the
...
Extends `higgsfield-seedance.md` § Frame Coordinate System with two demonstrated notations:
...
(`higgsfield-seedance.md` § Official Prompt Architecture — the Block Scaffold) applied
together; per-segment timing must still sum to the declared duration
(`higgsfield-seedance.md` § Output Format → Runtime arithmetic).
...
`negative-constraints.md` § Whole-Frame Degradation, which also covers
...
(`higgsfield-seedance-engine-rules.md`): the parenthetical tells the model the voice has no
...
- **Populated-plate reuse.** For a recurring crowded set (a restaurant),
  ...
  per shot. Use an empty plate when the model should direct the crowd
  separately; populate it once when the crowd must repeat. See
  `higgsfield-soul.md` § Variety Sheets — Crowds Without Clones.
