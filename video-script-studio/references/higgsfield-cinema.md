...
Before building a sequence, apply [shared director planning](narrative-planning.md). Identify events, performance arcs and state handoffs before assigning shots and duration. Map that plan into the selected Cinema version's settings strip, Elements and Single/Multi-Shot controls below. Scene/shot counters and total-duration limits are ceilings, not creative quotas; verify the selected version's limits and split generation clips when needed. Preserve Hero Frame identity, axis, action phase, lighting and audio at every join; a story stage does not automatically require a cut.
...
`higgsfield-pipeline.md` § Stage 8 — Assembly.
...
`higgsfield-pipeline.md` § The Edit's unification problem to solve at
...
The character sheet (see `higgsfield-soul.md` → Character Sheet
...
ratio stays fixed across 15 cuts; a character study where a signature accent
...
`higgsfield-moodboard.md`.

### Product Reference Sheet

When a product reappears across multiple shots — a recurring prop in a
narrative piece or a single item studied from a dozen angles — give it the
same asset-first treatment as a
...
|-------|-------|----------|
| Character | Identity, face, build, distinguishing marks | `higgsfield-soul.md` (Soul ID, character sheet creation) |
...
| Palette / Mood | Color and tonal mood across the whole project | This section + `higgsfield-moodboard.md` |
| Product | Product geometry, branding placement, material/surface, multi-view layout | This section |

Build the sheets the project actually needs. A short scene with one character in
...
A 480p draft is **not a cheap preview of the take you will re-render at 1080p**. The Seedance 2.0 schema exposes **no seed parameter** (verified — `model-specs.json` lists `resolution`, `mode`, `genre` and nothing else), so a higher-resolution re-run is a **fresh roll**, not a re-render. "Approve the take at 480p, re-render it at 1080p" is not a workflow the platform supports.
...
**The documented transfer mechanism for a look you want to keep is the Hero Frame** (`higgsfield-assist.md`) plus **start/end-frame pinning** (§ Keyframe Interpolation in this file) — pin the frame, not the roll. And fine-detail judgments (smoke threads, inscription legibility, reflections) must be **re-checked at final resolution**: 480p hides exactly the detail those judgments are about.

**Sound prompting:** When Sound is On, audio is generated natively alongside video in a single pass. For prompting audio behavior in detail (dialogue, SFX, ambient, music), see `higgsfield-audio.md`.
...
For Soul Cinema identity prompting and reference workflow, see `higgsfield-soul.md`.
...
Pick Cinematic Characters when: the face carries the shot (reaction beats, emotional climax, character intro close-ups), you are pulling micro-expression direction from `higgsfield-soul.md` (Deadpan Neutral, Quiet Devastation, Predator Glare, etc.), or wardrobe and styling specifics are part of the prompt and you need them rendered with fidelity. Avoid Cinematic Characters when the shot is wide and the character is small in frame (use Soul Cinema or Cinematic Locations), or when explicit camera-body / lens / focal-length control matters more than facial expression (use Cinematic Cameras). Pair naturally with the micro-expression library in `higgsfield-soul.md` and the Identity vs Motion separation rule when a Soul ID is active.
...
- **Soul Cinema identity prompting** — `higgsfield-soul.md`
...
- **Style/Director-Language Prompt Mode** — `higgsfield-seedance.md` (for Manual Style mode)
...
- **Audio prompting** — `higgsfield-audio.md`
...
- `template-*.md` — Genre-specific annotated templates
