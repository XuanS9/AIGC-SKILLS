...
For time-based transformations, use [shared director planning](narrative-planning.md): define the source state, transformation trigger, visible material response and final state, then allocate time. Style layers and preset stacks are not required story stages. Preserve identity, object structure and screen direction through each transformation; edits to an existing clip keep its source timeline and declared unchanged regions.
...
> see `negative-constraints.md` — Texture/Lighting Artifacts section.
...
- `template-*.md` — Annotated genre templates
