...
`higgsfield-seedance.md` § When the User Is Already in a Failure Loop instead —
...
3. Move the rephrased prompt through the preflight linter in
   `higgsfield-seedance.md` § Pre-flight Linter to catch other risk tokens.

Do not regenerate the same prompt unchanged — same text trips the
same classifier. See `higgsfield-seedance.md` § When the User Is Already in a
...
move finishes. Full treatment: `higgsfield-seedance.md` § Prompt-Craft Laws →
Motion-prompt laws.
...
3. **One speaker, one line, once.** The speaking character says the
   line and nothing else; `higgsfield-seedance-hell-grind.md` § Dialogue construction
...
4. **Every other visible face gets a positive at-rest mouth fact.** An
   ...
   still" — never as a negation (`higgsfield-seedance.md` § Prompt-Craft Laws → No
...
See `higgsfield-seedance.md` § Single-vs-multi-shot decision for the multi-shot
split mechanics.
...
See `higgsfield-seedance.md` § Spatial Layout Block for the full block structure
...
other (§ Context isolation in `higgsfield-seedance.md`).
...
blocking maps as first-frame geometry (`template-staging-reference.md`).
...
- `higgsfield-seedance.md` § Filter Model — read-this-first for filter rejection
  vs render failure distinction
- `higgsfield-seedance.md` § Voice Rewrite — language-level rewriting for filter
  passes (adjacent to § NSFW false-positive in this file)
- `higgsfield-seedance.md` § When the User Is Already in a Failure Loop —
  filter-loop recovery (sibling to this catalog for the
  render-failure side)
- `higgsfield-seedance.md` § Spatial Layout Block — block structure that prevents
  spatial-awareness failures
- `higgsfield-seedance.md` § Frame Coordinate System — coordinate vocabulary that
  the spatial layout block uses
- `higgsfield-seedance.md` § Single-vs-multi-shot decision — multi-shot split
  mechanics referenced from § Multi-motion camera overload
- `higgsfield-seedance-hell-grind.md` § Dialogue construction — the audio-block phrasing
  and the `INNER (unspoken)` marker that § Filler-babble on a short
  dialogue line depends on
- `higgsfield-troubleshoot.md` § Sequence & Continuation
...
- `higgsfield-soul.md` § Character Sheet Creation —
  ...
  drift failures
- `higgsfield-pipeline.md` § Master Production Chain —
