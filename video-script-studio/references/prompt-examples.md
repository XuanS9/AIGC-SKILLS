...
Continuation rules in `higgsfield-seedance.md` § Continuation
...
### Hospital Vigil — Prop Correction — Seedance 2.0 (Edit Shot)

```
Model: Seedance 2.0 (Edit Shot mode)
Aspect: 21:9 | Duration: 6s | Style: Naturalistic

[Source clip: the existing "Hospital Vigil II" generation above]

Change only the bedside lamp's shade from plain white to faded cream fabric.
Keep its shape, position, and warm light unchanged.

Preserve the husband's identity, bowed starting posture, hand on hers, and
his slow head lift followed by eyes filling, jaw tightening, and held breath.
Keep the original performance timing, composition, lighting, and shallow
depth of field throughout the full six seconds.

Camera: unchanged from source — locked-off medium close-up, no movement.

Audio: unchanged from source — the sustained low cello note swells through
his cracked breath without resolving. The heart monitor, distant intercom,
and rustle of bedsheets remain low beneath it, with their original timing.
```

Edit Shot mode is the targeted-patch tool. The prompt names exactly what to
change (the lampshade material and color) and explicitly preserves everything
else. The Keep Rule matters — without it, the generation drifts on every
variable that isn't locked.
...
> `higgsfield-seedance.md`, which is a Seedance-specific
...
`higgsfield-seedance.md` § Transformation for the construction
pattern.
