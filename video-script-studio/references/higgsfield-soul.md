...
For video, supply the approved identity and asset roles to [shared director planning](narrative-planning.md), then track changing expression, pose, outfit, props and damage separately through events. Preserve Identity Block / Motion Block separation and all reference workflows below; identity stays locked while motivated performance and state changes continue across shots.
...
✅ You want consistent faces across a multi-shot sequence
...
2. **Screen position** — qualitative anchor + percentage notation
   paired per `higgsfield-seedance.md` § Frame Coordinate
   System
...
characters share frame (see `higgsfield-seedance.md`
...
Photoshop composite" note in `image-models.md`.
...
> `higgsfield-facs.md` (FACS Action Unit codes). Each named expression
...
`higgsfield-seedance.md` § Reference Roles.)
...
crowd from the variety sheet. It gives you one plate that serves every shot,
and the crowd stays
directable per shot.
...
> `negative-constraints.md` — Face/Identity Artifacts section.
...
must be 4K finish in GPT Image 2 or Nano Banana Pro. See
`higgsfield-gpt-image-2-reference-sheet-workflow.md` § Views the video will need
for view coverage and the sheet-model ladder.
...
2. Add an explicit grade direction — "Bleach Bypass," "Teal Orange Epic,"
   ...
   Palette presets in `higgsfield-cinema.md` § Style Settings, or
...
- `template-*.md` — Templates 03, 04, 05, 06, 08, 09, 10 include Identity/Motion Block examples

## 整合版：写实参考板与交付验收

本项目默认执行 [角色资产交付规范](template-character-asset-delivery.md)，其约定优先于上文示例。默认一张完整角色资产参考图：左侧正面面部特写，右侧无头部的全身正面、侧面、背面，按需在右下方补充服装、配饰或道具的关键细节局部图。干净浅色背景，左右分区清晰，局部细节与主视图一致。正面定稿为身份主参考，其他面部角度按需单独补充并核对身份；多角度研究和 Soul ID 原有方法仍保留。

按规范的六行格式及阿德莱德案例交付精简完整提示词，不额外强调左侧特写数量。头脸特征与头部配饰写在左侧，身体服装与关键细节写在右侧，采用工作室灯光、电影级光影或柔和电影级边缘光、写实电影质感、8k、高清细节等描述；8k 不等于实际输出分辨率。摄影参数与分区皮纹仅按需要补充少量，不默认写长清单；不用磨皮、噪点、锐化或粗大毛孔制造细节，不强行做旧、不擅改定稿身份，不将案例外观套用所有角色。

已定稿面部在组板时直接沿用，新角度须核对身份；也可一次生成后验收。旧图或失败结果右侧残留头部时另存裁切修复版。真实场景另行定义光源与接触阴影；表情参考仅给同源生成备注，由用户单独生成，核对后用作表情辅助。身份符合定稿、右侧无头、局部细节一致性与视觉真实感均须看图验收。
