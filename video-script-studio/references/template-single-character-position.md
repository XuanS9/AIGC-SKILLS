...
in frame. Simpler companion to `template-multi-character-anchor.md` for cases
...
`template-multi-character-anchor.md` instead.

## Narrative planning

Use [shared narrative planning](narrative-planning.md): assets,
goal, global audiovisual/space, behavior, stages, continuity, final frame, constraints
are semantic checks mapped into this compact positional prompt. Let an event, causal
response, reveal, or state change determine any stage boundary, then allocate time.
No preset count or equal durations; one continuous action may be sufficient. Keep the
one-dominant-action and camera limits, carry contact, gaze, pose, and prop state through
the action, and end on the intended final frame. Fill any timestamp from that plan.
...
Goal and trigger: [why the character acts; intended visible result].
...
Continuity: [identity, direction, contact, and prop state; any motivated change].
Final frame: [composition and resulting character/prop state at last frame].
```
...
- **One dominant action + Motion** separation prevents the multi-
  ...
  `higgsfield-seedance-failure-modes.md` § Multi-motion
  camera overload

## See also

- `template-multi-character-anchor.md` (sibling template) — multi-character
  version with cross-character relationships
- `template-top-down-map.md` (sibling template) — meta-prompt to pre-visualize
