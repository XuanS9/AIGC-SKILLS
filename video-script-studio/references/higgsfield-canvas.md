...
For video graphs, apply [shared director planning](narrative-planning.md) before creating generation nodes. Keep one shared asset/goal/style/state plan, derive clips from events and the selected model's limits, then attach their real inputs and outputs to nodes. A node is a tool operation, not necessarily a story stage or a shot. Carry reference ownership and start/end state through dependency edges; retain the original collaboration, cost and execution workflows below.
...
episode becomes one continuous, re-runnable pipeline.

Canvas is its own top-level surface in the Higgsfield nav bar (alongside
Image, Video, Cinema Studio). It is model-agnostic: it
...
|---|---|---|
| **Soul 2.0** | photoreal images, fashion/editorial, consistent characters | `higgsfield-soul.md` |
| **Seedance 2.0** | flagship video, 1080p | `higgsfield-seedance.md` |
| **Kling 3.0** | cinematic video, multi-shot continuity, native audio | `higgsfield-motion.md` |
| **Wan 2.7** | high-action / dynamic motion | `model-guide.md` |
| **Veo 3.1** | Google flagship video, native audio | `model-guide.md` |
| **GPT Image 2.0** | 4K image gen, near-perfect text rendering | `higgsfield-gpt-image-2.md` |
...
So you can stage an entire scene pipeline and pay only for the renders you
...
- **Save any canvas as a reusable template** — for character
...
feed — see `higgsfield-workspaces.md` § Higgsfield Collab.
