...
At video execution handoff, compile [shared director planning](narrative-planning.md) into the actual prompts, bound assets, legal parameters and clip dependencies accepted by the selected tools. Keep planned narrative stages distinct from submitted jobs; carry output identity and join-state requirements back into review. The execution ownership, cost preflight and tooling contracts below remain in force.
...
Every Higgsfield generation costs credits, and one historical feature-production case reported roughly 1.0% image and 1.5% video acceptance rates (`production-benchmarks.md`); estimate the current task from its own scope and evidence. On Veo, Kling, Sora-2, and Seedance-class video, a single un-checked job can swallow hours of budget. The preflight pattern is part of the Tier 1 *Lock-before-generate* discipline (`production-checks.md`) — lock the cost estimate alongside the prompt, before submission, on whichever surface the user is on.
...
5. **Defer to their skill on execution flags.** If the user has `higgsfield-generate` loaded and asks for a cinematic scene, construct the prompt body here, then hand the prompt text off. Their execution layer verifies the current model schema and formats the invocation.

---

## Naming collision — `higgsfield-soul` (theirs) vs `higgsfield-soul` (ours)

Higgsfield's bundled-skills repo includes a skill named `higgsfield-soul` (renamed from `higgsfield-soul-id` in their v0.3.0). This sub-skill library also has a sub-skill named `higgsfield-soul` at `higgsfield-soul.md`. Same name, different jobs. The collision is real and worth disambiguating up front because both will trigger on user phrases like "Soul" or "Soul ID".

| | Theirs (`higgsfield-soul`) | Ours (`higgsfield-soul.md`) |
|---|---|---|
...
- When the user says "write me three Soul prompts for scenes A / B / C using my `reference_id`" — that is **our** `higgsfield-soul.md`'s job. Construct the prompts; do not try to run training.
...
> `Run python3 scripts/seedance_lint.py "<prompt>" before submitting to catch content-filter rejections. The filter is voice-based, not a keyword blacklist — see higgsfield-seedance.md for the full diagnostic.`
...
- Does not absorb their skills' logic. `higgsfield-generate` owns generation invocations.
...
- `higgsfield-prompt.md` — produces the prompt text that gets handed off to any execution surface.
- `higgsfield-workspaces.md` — handles the upstream "which Higgsfield workspace fits my task" question, which is settled before any execution surface comes into play.
- `higgsfield-seedance.md` — pairs with the `../scripts/seedance_lint.py` preflight recommendation above; covers the full filter diagnostic and prompt-mode router.
- `higgsfield-assist.md` — credit-optimization questions apply regardless of execution surface; route credit/plan questions there.
- `higgsfield-soul.md` — our Soul prompt-construction sub-skill. Read the Naming collision section above before routing — theirs trains, ours prompts.
