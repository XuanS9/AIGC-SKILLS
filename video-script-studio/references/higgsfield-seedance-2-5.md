...
- Emotion uses selected **observable** cues (2–4 is an example, not a quota); niche camera terms get translated into a visible result [→](#emotional-direction-and-camera-terms)
...
| `[OFFICIAL — platform]` | Higgsfield's live `models_explore` catalog, snapshot **2026-08-07** (`model-specs.json`). Parameters, enums, and media roles come from here and nowhere else. |
...
## Shared narrative planning

Apply [shared narrative planning](narrative-planning.md) across all four modes,
including first/last frames, keyframes, storyboards, blockouts, one-click video, and
transitions. Establish the goal and ending, then derive stages from story events,
causality, reveals, and state transitions before allocating duration. Do not preset a
stage count or equal-length intervals. A **stage is not a cut or a generation clip**:
...
subject follow-through, visual result, camera path and sound, with an inherited end state.
Single-shot and quiet scenes retain this detail. Explicit native-format requests compile
that content into role maps, bracket audio and mode-specific orders without duplicate
long prose; scoped edits keep their scope. Keep platform settings separate. Budget
transitions and the final frame, and preserve results until a motivated change.

For `video_edit`, source events and timestamps determine edit annotations: preserve the
...
> `higgsfield-seedance-2-5-vfx-pipeline.md` § Stage 4. `[FIELD — AI-vs-VFX, 2026-08-08]`

---

## The Higgsfield Parameter Surface

`[OFFICIAL — platform, snapshot 2026-08-07]` · verify against `model-specs.json`
...
The linter reads the enums out of `model-specs.json`, so an out-of-range duration, a
...
`[OFFICIAL — Dreamina]` This compact formula supports optional short submissions and
native-field compilation. Use the parts the shot needs without padding; the complete
performance master retains the shared format and detailed causal development.
...
- **Visual style** — lighting, color, materials, texture, mood. Only descriptors that add
  ...
  The named-substitute discipline in `higgsfield-seedance.md` § Prompt-Craft Laws
  applies unchanged.
...
**Generation parameters are separate from prompt text.** Set supported resolution,
duration and aspect ratio on the generation page or API; respect inherited/locked values
in the relevant mode. Prose time ranges still allocate performance and handoffs, but do
not set API duration or promise exact frame timing.
...
- **When a reference video already carries the motion accurately, specify which
  attributes to inherit in the role map.** Avoid conflicting re-choreography in the
  submission; retain the inherited causal action, responses and join states in the master. A blockout or motion
...
Each stage carries **one primary state change**, its visible response, and an **explicit,
directly visible end state**. Supporting reactions and micro-actions serve that change.
Each new stage restates what carries over. Derive the necessary stages before estimating
their durations; neither 30 seconds nor this example implies three stages or equal thirds.
The skeleton below illustrates one possible progression; add or remove stages as needed.
It is a planning/compilation skeleton, not a complete deliverable: expand each event into
the shared stage prose with action, response, follow-through, camera, sound and handoff.
...
incompatible jobs in one generation**, and that can justify a separate generation and an editorial cut
...
In that case, split into **two prompts, one job each, stitched in the edit**.
This is a worked remedy, not a universal two-prompt composition rule:
...
**How to tell which split you need.** Duration-driven splits follow the time
needed for the events, responses, dialogue, and transitions, checked against the
model cap. Complexity-driven splits ask whether the generation can render its
jobs together clearly. Two jobs do not automatically mean two shots: compatible
action and reaction may stay in one take. Split where a causal/state handoff or
coverage change protects the performance, and record the inherited join state.
...
The same production reported that it could not obtain a perfect 30s render;
that is a production observation, not a model prohibition. Generate footage per
compatible job and assemble when the content needs that split. Several
...
Stages are the default. Allocate their durations after the event/state plan, using
performance and reading time rather than equal division. Full performance stages use
time/event ranges; add exact targets when a handoff, entrance/exit, transition or beat
needs a cue, without claiming frame precision. For editing,
retain source timestamps instead of making a new stage clock; attached audio keeps its
own timing. State cuts separately from stages.
...
- **Do not infer a per-second action quota from timestamps.** Preserve user-requested
  cadence and causal choreography as timing targets, assess readability and actual mode
  limits, and verify the render/post timing; prose does not guarantee exact cadence.
- Every timestamped segment still ends on an explicit end state, exactly like a stage.

> **Relationship to 2.0's beat arithmetic.** `higgsfield-seedance.md` § Output
...
Two house rules carry over from `higgsfield-audio.md` and the film pipeline in
...
- **Diegetic-only is a project choice, and 2.5 finally obeys it.** Random subtitles and
  ...
  lead with the positive diegetic list before it (`higgsfield-audio.md` §
  Suppressing music).
...
The long templates live in **`higgsfield-seedance-2-5-mode-playbooks.md`** in this directory. What you need to know
before opening it:

- **Video editing** — define the source as the **sole editing master**, then edit goal, edit
  scope, target material, and the preserve list, including original event timestamps,
  duration, cuts, and audio timing except where explicitly edited. Subject replacement additionally needs a
...
an optional example, not a quota**; choose meaningful cues visible at the current scale
and triggered by the event. Preserve the complete action/response arc without listing
every facial detail or forcing close-ups.
...
For muscle-level control beyond this, `higgsfield-facs.md` (AU codes); for the
behavior-under-pressure layer that makes the cues *mean* something,
`higgsfield-acting.md`.
...
> `higgsfield-seedance.md` § FOV anchors is house doctrine across the Seedance
...
- **Eyes / soul** — what the gaze is *doing* and what it carries. Dead eyes are the number-one
  tell; see `higgsfield-acting.md` § Eye life.
...
reason `higgsfield-seedance.md` § Drafts Validate the Prompt gives: there is no
seed to pin.

### What carries over from 2.0 unchanged

Use `higgsfield-seedance-engine-rules.md` as legacy prompting and remediation guidance,
subject to [shared narrative planning](narrative-planning.md) and actual 2.5
mode limits. Its exit/re-entry, off-screen, reflection, tracked-cast and double-contrast
suggestions do not become universal 2.5 prohibitions or forced cuts. Preserve explicit
identity, causal action and off-screen state handoffs. Observable microexpression cues,
positive phrasing, homograph checks and production block scaffolds remain useful; none
replaces the shared complete-performance format or guarantees rendering.
...
| **Clay Renderer plugin** — white-model rendering workflow | Not exposed as a plugin | Coarse/fine blockout prompting via `omni_reference` (`higgsfield-seedance-2-5-mode-playbooks.md` § Blockout references) |
| Native 180s / multi-minute deliverables | — | Derive pieces from events and join states, each within the 4–30s generation range, and assemble in post (`higgsfield-pipeline.md`) |
...
`higgsfield-seedance.md` § Extension Prompting.
...
- [ ] Stages follow events/causality/reveals/state transitions; durations follow performance needs, with no preset count or equal division
- [ ] Stages, camera cuts, and generation boundaries are separately justified; final frame and transitions fit the total
- [ ] Each stage has a primary change, visible response, inherited state, and clear end state
...
- `higgsfield-seedance.md` — Seedance 2.0 director: the filter model, block scaffold,
...
- `higgsfield-acting.md` — performance: objective, obstacle, tactics, beats, eye life
- `higgsfield-facs.md` — facial control by Action Unit code
- `higgsfield-seedance-vfx.md` — video-to-video footage transforms on 2.0
- `higgsfield-models.md` — model choice against the specs layer
- `higgsfield-pipeline.md` — assembling 30-second pieces into a longer deliverable
- `higgsfield-seedance-2-5-mode-playbooks.md` — the editing / extension / one-click / transition / blockout templates
- `higgsfield-seedance-2-5-vfx-pipeline.md` — the AI-VFX production pipeline: asset-class model routing, the size-ref
  frame, location batching, the `omni_reference` v2v lane, the four-batch rule, the slop catalog

## 整合版：人物身份与表情参考的职责

本项目执行 [角色资产交付规范](template-character-asset-delivery.md)。默认图左侧是正面面部特写，右侧是无头部的全身正面、侧面、背面，按需附服装、配饰或道具的关键细节局部图；干净浅色背景，左右分区清晰。正面定稿为身份主参考，其他面部角度按需单独补充并核对身份。右侧身体与局部图只负责体型、服装、道具及对应细节，均属于同一角色，不能把不同视图识别成多个人或将局部图中的物品算成新增道具。旧图或失败结果右侧残留头部时按需另存裁切修复版。强表情图只交同源生成备注，由用户另行生成，默认不混入设定板，也不引用尚未上传的表情素材。用户补充并核对表情图后，只赋予其表情肌肉、嘴形和牙齿结构的辅助职责，不改变定稿身份。

提示词必须说明各素材属于谁、负责什么、排除什么；灰底、多视图排版和身体图裁切不进入成片。中性资产用于固定形体与材质，视频场景自行定义光照；写实质感从通过视觉验收的主参考延续，不靠视频提示词反复描述五官来重造身份。继续执行原阶段状态、参考映射、模式参数及 preflight；静态 lint 通过不能证明面部一致性或视觉真实感。

## Project convention — cross-genre director scaffold

For this project's Seedance 2.5 prompt writing, apply [shared narrative planning](narrative-planning.md) in every mode, then read both the [跨类型导演模板](template-general-director-2-5.md) and the entire [user-approved full performance example](template-full-performance-example.md). Default to a complete, copyable performance brief: asset roles, overall goal, global audiovisual/spatial rules, behavior and pacing, detailed staged performance, state inheritance, final frame and scoped constraints. Within each stage, develop the action, visible response and subject follow-through in concrete prose; specify visual layers, camera path, sound, resulting state and next-stage handoff. Single-shot work follows the same visible headings and retains full performance detail. Only an explicit concise request, scoped edit or target hard limit calls for a shorter deliverable/submission; preserve the complete master for full tasks. Runtime, stage count, speed, camera movement, effects and genre remain task-specific. This full-performance default takes precedence over short-form style recommendations while retaining all native formats and capabilities.

Bind actual uploaded references only; use stable subject names in action prose. Preserve accumulated results until an explicit narrative change, including handoffs, cleanup, scene changes, and restoration where appropriate. Distinguish in-world symbols and requested branding/subtitles from unwanted overlays. Keep platform settings separate from prose, verify the actual hosting surface, and treat beat timestamps as timing targets rather than frame-accurate guarantees. Editing and extension retain their mode-specific master/scope and boundary contracts in MODE-PLAYBOOKS.md. This is a user-derived composition convention, not a new model capability claim or a measured generation result.

Project runtime preference: inherit explicit film and sequence budgets; otherwise estimate natural duration from performance workload. There is no 30-second or 15-second creative default and no near-30s target for shots or generation clips. Apply content-led segmentation in narrative-planning.md: choose the viewing purpose and motivated cut, estimate retained shot duration, then group or split legal generations while preserving action phase and continuity. Keep generation duration distinct from retained edit duration; never pad to a model limit. Editing preserves the source timeline; extensions budget only their new material. Actual platform/mode limits remain independent and require verification.
