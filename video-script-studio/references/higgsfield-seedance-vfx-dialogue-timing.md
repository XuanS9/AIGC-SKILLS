...
[`higgsfield-seedance-vfx.md`](higgsfield-seedance-vfx.md) § Timed camera moves synced to dialogue.
...
- **Recompute on every runtime change.** If the user changes the total duration, every numeric
  ...
  back; do the `total − intro = surviving window` subtraction (`higgsfield-seedance-vfx.md` § Duration
...
extraction trap are documented in [`higgsfield-audio.md`](higgsfield-audio.md)
