...
`higgsfield-seedance.md` instead.
...
Use [shared narrative planning](narrative-planning.md) to inspect
the source before writing. Its eight layers — assets/master, goal, global audiovisual
and space, behavior, stages, continuity, final frame/handoff, constraints — organize
the edit semantically; keep the plain-text English output, without markdown inside it.

Lock the source timeline and mark changes at its events, causal triggers, reveals, or
state changes. Preserve timing, performance, cuts, and unaffected boundary states;
do not repartition the source into equal stages. Allocate time only for authorized new
content or moves, after planning their causes and arrival states. All worked-pattern
runtimes, timestamps, and creature counts below are illustrative, not mandatory; inspect
the real clip and retain the selected mode's runtime and fidelity constraints.
...
show → name the one change and its source-time trigger → lock everything else → define
the resulting effect state at the final source frame → close with the lock-down clause.
If added content changes the assembled timeline, label source time separately from
output time and account for the join without retiming the preserved performance.
...
framing with correct lip-sync. This pattern explicitly adds an intro and camera reveal;
it is not the default source-preserving edit. Plan the reveal-to-source join first,
including pose, action phase, speed, framing, light, and sound. The illustrative 8s total
and ~2s reveal leave about 6s for source performance; choose the actual budget only after
inspecting the source, and identify any omitted content before using a shortened window.
...
- **Match the source runtime** by default. Any authorized intro or runtime change needs
  an explicit source-to-output time mapping; recompute numeric zoom marks without
  silently retiming or dropping the preserved performance.
- **Prepended intro?** `total − intro = surviving window` for the source performance — flag
  what falls off before promising lip-sync (`higgsfield-seedance-vfx.md`
  § Duration discipline).
- **Creature still reads CG?** Add a real reference photo as `@creature` (texture only), or
  generate the transformed start frame first (`higgsfield-seedance-vfx-first-frame.md`).
