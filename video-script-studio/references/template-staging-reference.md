...
This is the companion to `template-top-down-map.md` and the opposite half of it. That template is
...
> blocking locks in `higgsfield-seedance.md`, not with the map alone.
...
For video blocking, use [shared narrative planning](narrative-planning.md).
Its eight semantic layers — assets, goal, global audiovisual/space, behavior, stages,
continuity, final frame, constraints — inform the shot; this helper supplies spatial
anchors in its existing drawing prompt and connector format. Keep all three anti-bleed
layers and attachment order above; do not add planning or graphic vocabulary to the
video prompt merely to enumerate the layers.

Choose movement boundaries at events, causal triggers, reveals, or state changes, then
allocate their timing. No preset count or equal durations. A new stage need not mean a
new map or cut: hold the initial positions until the scripted trigger, describe the path
and response, and carry end positions/contact/prop state into the next shot. The final
map, if needed, represents the planned final frame. Measurement sample counts above
remain evidence about the method, not creative stage or shot quotas.
