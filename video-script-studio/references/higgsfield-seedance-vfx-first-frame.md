...
from [`higgsfield-seedance-vfx.md`](higgsfield-seedance-vfx.md) § First / start-frame workflow.
...
2. **Generate the transformed still.** Use an image model that takes the source frame plus any
   reference as input:
   - `higgsfield-gpt-image-2.md` (GPT Image 2) or Nano Banana Pro — good at holding a
...
4. **Hand the still back to Seedance as `start_image`.** Upload the approved still and declare it
   ...
   (`model-specs.json`). The video prompt then describes **motion + camera only** from
...
- Reference handles are assigned by upload order (`@Image1`, `@Image2`, …). Keep a stable
  ...
  reference carries which property (see `higgsfield-seedance.md` § Reference Roles /
...
quality." Full treatment: `higgsfield-seedance.md` § Drafts Validate the Prompt, Not
the Take.
