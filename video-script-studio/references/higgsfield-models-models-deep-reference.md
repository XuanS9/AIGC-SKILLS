...
Use [shared narrative planning](narrative-planning.md) to plan the story
before mapping it to model calls. A **stage** is a dramatic event or state
transition; a **shot** is continuous camera coverage between cuts; a **generation
clip** is one model output. A shot can span stages, a stage can need several
shots, and a supported multi-shot clip can contain several shots. Record entry
state, main change, visible response, and exit state across these boundaries.

Duration ranges, discrete duration settings, reference budgets, and multi-shot
caps in this reference are **platform constraints**, not prescribed story
structures. Choose stage and shot lengths from action, dialogue, information,
and performance; then fit generation clips to the chosen model/mode. Do not
infer cut count from runtime, default every clip to 15s, or force equal lengths.
For a shorter editorial beat, trim a legal generation; for a longer event, use
supported extensions or connected clips with explicit state handoffs. Track
retained edit time separately from raw generation duration.
...
**Multi-shot generation:** Up to 6 camera cuts in a single generation — a capability ceiling, not a required count. Per-shot control over duration, shot size, perspective, narrative content, and camera movement. Describe an entire sequence in one prompt — model generates a coherent multi-shot video with edited camera cuts, not just a single clip.
...
**Text rendering:** Legible signage, logos, labels preserved throughout motion sequences.
...
and voice → generate a 15s dialogue scene with your AI double in a studio setting.
→ Kling 3.0 Omni
```
...
> For the Higgsfield UI workflow and motion-reference input checklist, see `higgsfield-motion.md` → "Kling 3.0 Motion Control — When and How to Run It" and "Motion Reference Input Checklist".
...
Use this format for multi-shot sequences with per-shot control. The four-shot
example is optional coverage; add, remove, or combine shots according to the
story. Assign each duration independently and reconcile the sum to the selected
clip duration and model's multi-shot limit:

```text
Shot 1 ([DURATION_1]s): [Wide establishing shot, if needed]. Camera: static.
Shot 2 ([DURATION_2]s): [Medium shot, action begins]. Camera: slow push in.
Shot 3 ([DURATION_3]s): [Close-up, reaction or detail]. Camera: static.
Shot 4 ([DURATION_4]s): [Resolution]. Camera: tracking / pull back.
```
...
> term; the four brief mentions of "Rule of 12" elsewhere (in `higgsfield-audio.md`,
> `higgsfield-models.md`, and twice more in this file below) are uses of this
> now-sourced term.
...
2. ACTION    — primary causal action + physics/timing (include its response and result)
...
> suspect. Sourcing detail: `higgsfield-audio.md` § Audio by Model →
> Seedance 2.0.
...
Too many independent events in one clip can cause jitter and morphing. Use the
following historical ranges as **optional overload diagnostics**, not maximum
capacities, target counts, or a runtime-to-segmentation formula. Define causal
stages first; their complexity and visible consequences determine how much time
they need. Multiple linked gestures can serve one change, while a single reaction
can need a long uninterrupted performance.

| Illustrative clip duration | Heuristic primary changes (not a quota) |
|---|---|
| 4–6s | 1 primary change |
| 8–10s | 1–2 changes |
| 12–15s | 2–3 changes (use timestamps) |

The older Level 4 fight example cites 6–8 beats per 5 seconds with precise
per-beat specification. Treat this as an optional dense-choreography example,
not guaranteed capacity or a required pace; micro-actions are not equivalent to
primary state changes. Verify contact, response, recovery, and spatial continuity.
If overloaded, simplify, lengthen within the supported range, or split at a
clear state handoff. Seedance 2.0's **4–15s per generation** remains a hard range;
stages, shots, and complete scenes do not inherit that range.

---

#### Genre Templates — Ready-to-Use Starters

**Object study (Level 2, T2V/I2V):**
```
...
```
Example use case: Fast concept video — 5 visual directions for a short film,
...
```
Use when: Rapid concept testing and draft scene generation
that need speed over maximum quality.
→ Veo 3.1 Fast
```
...
- Illustrated documents requiring precise text/logo rendering
...
### GPT Image 2
**Best for:** Photorealistic commercial imagery · text-heavy and multilingual designs · character consistency across multi-shot sequences · native 4K output
...
- Multilingual layouts where non-Latin scripts must render legibly
