...
For video tasks, carry the goal, asset roles, required events and final state from [shared director planning](narrative-planning.md) through workspace routing. Choose a surface with the controls the task needs, then compile the plan into its native format. Workspace switches are tool decisions; they do not automatically create story stages or cuts. Keep the image, audio and other workspace routes below.
...
- Any time you need a voice *asset* that will be fed into a downstream
  workspace (a finished track to feed into Lipsync Studio, narration to
  underlay a Cinema Studio sequence).
...
layer that sits across the workspaces above. It is where a scene
...
> shared spaces, chat, and calls. **Shared Canvas** (see
> `higgsfield-canvas.md` § Shared Canvas) is board-level live
> co-editing of a single node graph. Reach for Collab to organize a team and
> a production; reach for Shared Canvas to build one pipeline together in real
> time.
...
prompt and a Sora 2 Trends prompt both benefit from MCSLA
