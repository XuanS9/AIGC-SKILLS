...
`higgsfield-seedance-2-5.md` in this directory is the **dialect** — modes, reference-role grammar, staging,
...
> the grammar of `higgsfield-seedance-2-5.md`. The block scaffold (`SCENE CONTEXT → … → POSITIVE LOCKS`),
> FOV-in-degrees, distributed style, the CAMERA-3rd-position rule and the prompt-craft
> laws are house doctrine in `higgsfield-seedance.md` and carry across to 2.5
...
| `[OFFICIAL — platform]` | Higgsfield's live `models_explore` catalog, snapshot **2026-08-07** (`model-specs.json`). |
...
This composes with, and does not replace, the reference-role discipline in `higgsfield-seedance-2-5.md` —
...
> This is the 2.5 form of the scale-drift lock in `higgsfield-seedance.md`
...
`higgsfield-seedance.md` § Positive Locks.
...
ignores `duration` entirely and bills by the master's length (`higgsfield-seedance-2-5.md` § The Higgsfield
Parameter Surface).
...
| Change one scoped thing inside a master whose timeline must survive untouched (a wall's light colour from 4–7 s, remove the music) | `video_edit` — `higgsfield-seedance-2-5-mode-playbooks.md` § Video editing |
| Rebuild the shot from the ground up, old clip as motion guidance only | `omni_reference` — `higgsfield-seedance-2-5.md` § The Mode Router, rule 2 |
| Plate is Seedance **2.0** work (4K needed, platform start/end frame, genre hint) | `higgsfield-seedance-vfx.md` |
...
Inheritance clause in `higgsfield-seedance-2-5-mode-playbooks.md`:
...
This is the same law as `higgsfield-seedance.md` § Prompt-Craft Laws — write the
...
sprint. Beat-level acting direction lives in `higgsfield-acting.md`; this is the
...
- `higgsfield-seedance-2-5.md` — the 2.5 dialect: modes, reference roles, staging, bracket syntax, limits
- `higgsfield-seedance-2-5-mode-playbooks.md` — `video_edit` and `video_extension` order forms, storyboards, blockouts, transitions
- `higgsfield-seedance.md` — house prompt doctrine: block scaffold, FOV anchors, distributed style, prompt-craft laws, positive locks
- `higgsfield-seedance-hell-grind.md` — the feature-film pipeline (asset construction at film scale, iteration discipline)
- `higgsfield-seedance-vfx.md` — the **2.0** video-to-video lane (4K, `mode=std`)
- `higgsfield-character-design.md` — what goes on the sheet before any model runs
- `higgsfield-acting.md` — performance construction for the emotion beats
