...
one model output. A shot can span stages, a stage can require several shots,
and a supported multi-shot clip can contain several shots. The numbered
**production stages** below are workflow steps, not story divisions or clip counts.

Identify entry state → main change → visible response → exit state, choose
coverage, then allocate durations and generation calls. Events, causal turns,
performance arcs, locations, and continuity handoffs determine boundaries; total
runtime does not prescribe cut count or equal lengths. Keep stage timing, shot
lengths in the edit, and model-supported generation durations separate. Carry
identity, props, positions, emotion, and environmental results across handoffs.
...
- Pipeline E: story-led segmentation; Seedance 2.0 supports 4–15s per generation; a style or scene can span clips; carry previous accepted video plus identity/state anchors [→](#stage-5--seedance-20-with-keyframe--previous-video)
...
- Never describe character age in Seedance prompts; longer scenes need connected legal clips, not a 15s scene cap [→](#pipeline-e-pitfalls)
- Reference-based continuation: attach the accepted clip + open with "The scene continues."; match resolution for continuity and choose a supported duration for the next event [→](#the-extend-a-clip-workflow)
...
- Use a motivated camera-angle change when a join needs coverage; preserve continuous takes when needed and plan transitions ahead (last-channel-on-TV trick) [→](#clean-join-planning)
...
> see `higgsfield-assist.md`. For upstream project
...
> — see `higgsfield-prompt.md` § Identity vs. Motion
> Separation Rule and `higgsfield-soul.md` § Identity vs.
...
Every scene should have one clear overall purpose, which may develop through
several narrative stages and shots. Overloading one generation with unrelated
events can make the AI blend details — messy motion, wrong angles, extra
people, broken hands, missing objects, confusing results. Keep each stage's
main change legible and map it to coverage and clips as needed. Six scene
purposes cover most project work:
...
> **Action clarity at the right unit.** Step 06 addresses the narrative purpose
> of a scene, which may span multiple stages, shots, and clips. The one-primary-
> action guidance in `higgsfield-prompt.md` § One Action Per Scene is
> useful for simple single-shot prompts; do not turn it into a required cut per
> gesture or equate a scene with one generation. Keep setup, response, and result
> connected; split when causal clarity or model limits require a handoff.
...
> `higgsfield-soul.md` § Identity vs. Motion Separation
> (also `higgsfield-prompt.md` § Identity vs. Motion
...
> documented in `higgsfield-prompt.md` § The Iteration
...
> five-verdict triage in `higgsfield-troubleshoot.md`
...
- Pass 3: Scene breakdown into narrative stages and state handoffs
- Pass 4: Shot list with story-led coverage and edit durations
- Pass 5: Image prompts for required identity, location, and boundary anchors
- Pass 6: Video prompts mapped to supported generation clips
...
> type (Cinematic Short Film, Character Series, Fast Iteration,
> Multi-Style Short Film).
...
5. Plan each scene's stages, shots, and state handoffs; write image prompts for the required anchors.
6. Map the shots to video prompts with model-supported clip durations and intended edit lengths.
...
[6] VIBE MOTION      → Add motion graphic layers (titles, captions, credits)
...
Add motion graphic layers — titles, captions, lower thirds — on top of
...
- Location/time stamp
- End card with film title or credits
...
6. Verify final runtime and continuity, then export for platform
```

Use the retained shot lengths to reconcile the final edit to the requested
runtime, including transition overlaps and any retiming. Raw generation lengths
can total more than the finished film; trim unused handles and do not pad story
beats to consume every generated second. Final holds belong inside the runtime
budget and are used only when the ending needs them.
...
still moments at start and end. See `higgsfield-cinema.md` §
Multi-Shot Manual → Morph-Cut and Smooth-Cut breathing room. These are edit
handles for that transition technique, not mandatory story holds; budget them
within supported generation lengths and trim or overlap them in the edit.

---

## Pipeline B: Character Vignettes (Streamlined Chain)

**Goal:** A series of short scenes with the same character and aesthetic
**Credits required:** Medium (Pro plan)
**Time:** 30–60 minutes per scene once Soul ID + Moodboard are set up

```
[1] SOUL ID          → Character locked once, reused forever
[2] MOODBOARD        → Aesthetic locked once, appended to every prompt
[3] GENERATE IMAGE   → Soul 2.0 or Nano Banana 2 for each scene
[4] ANIMATE          → Kling 2.6 I2V, simple motion per scene
[5] VIBE MOTION      → Optional scene titles or captions
```

**Per-scene prompt template (after Soul ID + Moodboard are established):**

```
[Soul ID character] is [specific action] at [specific location].
[One specific visual detail that changes this scene from the last.]
Camera: [simple control — Dolly In / Arc / Overhead].
Aspect: 9:16. Duration: [model-supported length for this scene's action].
[Moodboard style modifier — same every scene.]
```

**Example series (3 scenes, same character):**
```
Scene 1: [Soul ID] sips coffee at a sun-drenched café terrace. Reading a book.
         Camera: Dolly In. Aspect: 9:16.
         Style: warm amber, shallow DOF, golden hour.

Scene 2: [Soul ID] walks through a quiet morning market, tote bag on shoulder.
         Camera: slight Arc. Aspect: 9:16.
         Style: warm amber, shallow DOF, golden hour.

Scene 3: [Soul ID] sits at a desk by a tall window, writing in a journal.
         Camera: Dolly In. Aspect: 9:16.
         Style: warm amber, shallow DOF, golden hour.
```
...
## Pipeline D: Fast Creative Iteration
...
[4] VIBE MOTION      → Add titles or captions if needed
[5] EXPORT           → Export the selected test clips for review
```
...
**Time:** Historical example: 2–4 hours for an 8-scene short; scene and clip counts follow the current story.

**The core trick:** feed the **previous accepted clip's video + prompt** into the next unit's planning step. A scene or style can span several clips. Carry character, prop, and story state explicitly while style changes at narrative boundaries; use canonical identity references alongside the video.
...
[4] CLAUDE           → Plan stages/shots, then clip prompts using keyframe + previous accepted prompt
[5] SEEDANCE 2.0     → Animate each legal clip with keyframe (style ref) + previous video (continuity ref)
[6] REPEAT as needed → Carry accepted exit state into the next clip; re-anchor at scene/style changes
...
This is the stage most people skip and it's where the consistency comes from. For each scene and any connected continuation:

1. Upload the relevant keyframe (from Stage 1–2)
2. **Also upload the previous accepted clip's video prompt and result** so Claude can compare the intended state with what actually happened and carry through effects and wardrobe state
3. Describe the scene in plain language — what the hero does, the visible response, the causal turns, and how it ends
4. Claude plans the narrative stages and shots, then outputs as many Seedance-ready prompts as needed. Assign each generation **4–15s** according to the action and performance load; a scene can exceed 15s across connected clips. Record each clip's entry/exit state and intended edit use.

```
Example plain-language brief to Claude (scene 3, chibi arena; 15s is this example's requested edit runtime, not a template):
...
Claude, given the previous prompt + result + keyframe, carries the teleport effect, watch behavior, and hero identity into the new prompts. Check that this example's action is readable in the requested edit time; simplify redundant beats or use multiple legal clips and trim, rather than forcing the whole list into one generation.

### Stage 5 — Seedance 2.0 with keyframe + previous video

For each generation clip, feed Seedance these input roles:
1. **The Claude-written prompt** (from Stage 4), including the new event and continuity handoff
2. **The keyframe** as style reference, plus canonical character/prop references where needed
3. **The previous accepted clip's video**, when available, as world/motion continuity reference; for the opening clip, use the planned opening-state anchors

**Platform constraint:** Seedance 2.0 supports **4–15s per generation**; reference
assets must also fit its count and duration budgets. This is not a cap on a
scene, a required duration, or one generation per style. Keep the chosen style
consistent for as many clips as the story needs. Use internal cuts only where
coverage calls for them; a continuous take is equally valid. A shorter editorial
beat can be trimmed from a legal clip. If a unit exceeds the limit, split at a
readable causal or performance handoff and preserve exit/entry state.

Video references carry world, energy, and effects; they do not guarantee exact
identity or frame-accurate continuation. Pair them with identity images and
explicit opening action or boundary-frame anchors as supported by the mode;
see `higgsfield-camera.md` § Video Reference — What It Reads, and What
It Can't.

### Stage 6 — Repeat with feedback loop

For each subsequent clip, use the last **accepted** result, inspect its actual ending state, and update the next entry state. Continue within a scene or open a new scene as the story requires. Keep the style/identity anchors stable, and use the chain-depth and re-anchoring guidance below to prevent accumulated drift.

### Stage 7 — Assembly

Export the accepted clips into your NLE in story order. For the teleport example,
crossfade on the flashes where useful and add music under; choose other joins
when the story needs them. Trim each clip to its intended shot lengths, preserve
state handoffs, and reconcile the final runtime including overlaps and the ending.
The historical 8-scene example does not prescribe eight clips or equal durations.

### Pipeline E pitfalls

**Pitfall 1: Skipping continuity inputs.** When continuing an accepted event, use the previous video for world/motion continuity and canonical character/prop references for identity. Inspect the actual exit state and specify the next action; a video reference alone cannot guarantee the hero's likeness or exact continuation.
...
**Pitfall 6: Treating the clip limit as a scene template.** Seedance 2.0 supports 4–15s per generation. Longer scenes need as many connected clips as their events, performances, and state handoffs require, with independently chosen legal durations. Do not force a split in two, equal lengths, or extra cuts to fill 15s.
...
`higgsfield-seedance.md` — this section does not restate
...
lives in `higgsfield-troubleshoot.md` § Sequence &
Continuation Failure Atlas — one repair variable per symptom.

### The extend-a-clip workflow

[EMPIRICAL — cross-surface] For surfaces without a dedicated extend button,
use this reference-based continuation workflow. When the selected model/mode
provides native extension, follow its actual parameters instead:

1. **Attach the accepted clip as a video reference.**
2. **Open the prompt with "The scene continues."** State what happens next,
   using the accepted exit state to plan the handoff. The reference can carry
   world, motion character, and voice style, but exact continuation requires
   review and suitable anchors. For a prequel clip, open with **"Show me what
   happens before"** and plan its ending against the source's opening state.
3. **Match the source resolution where supported** (1080p→1080p) to avoid a
   quality jump. Choose a legal duration for the next event's workload; matching
   the source's duration (e.g. 15s→15s) is an optional historical workflow choice,
   not a general requirement. Follow any native extension mode's actual fixed
   duration or resolution constraints.
...
- **Use a motivated camera-angle change when the join needs coverage** ("the
  scene from the character's perspective"). It is an optional seam-management
  technique; preserve a continuous take when the performance or event needs it.
...
> See `higgsfield-seedance.md` § Output Format.
...
| Recurring character vignettes | Pipeline B (Character Vignettes) |
| Quick creative tests | Pipeline D (Fast Iteration) |
...
> scene continuity breaking, camera contradictions), see `negative-constraints.md` —
...
- `template-*.md` — Annotated genre-specific templates for pipeline starting points
