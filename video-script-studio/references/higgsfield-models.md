...
For video selection, use [shared director planning](narrative-planning.md) to estimate the actual event/performance workload first, then compare supported modes, durations and reference controls. Model duration tables are legal ranges or enums, not recommended story-stage lengths. Keep stages, shots and generation clips distinct; split at compatible state handoffs if the selected model cannot fit the required content. Preserve all model-specific controls and verify stale specs as required below.
...
specifics, parameters, edge cases, API details) → read `higgsfield-models-models-deep-reference.md`.
...
For deep documentation on any specific model → read `higgsfield-models-models-deep-reference.md`
...
- `template-*.md` — Annotated templates with per-genre model recommendations
