...
- **Double contrast** — a cut technique changing BOTH shot size AND camera character (handheld / static / stabilized tracking / crane / aerial) when the event benefits from strong contrast. Intentional repeated coverage and match cuts remain valid; this technique does not require extra cuts.
...
- **`[0-Ns]` bracket notation** — time-range prefix for shot phases inside a single continuous shot. Example: `[0-3s] subject enters frame, [3-7s] camera pushes in`. Used for per-second beats inside single-shot continuous motion (see `higgsfield-seedance.md` § Single-vs-multi-shot decision).
...
Specifying each layer separately prevents the multi-motion overload failure mode (see `higgsfield-seedance-failure-modes.md` § Multi-motion camera overload).
...
- `Static locked-off camera. Zero movement. No pan, no zoom, no dolly, no shake.` — atmospheric / observational shots, prop close-ups, sacred-register imagery
...
| High-key | Bright, minimal shadows (comedy, lighthearted scenes) |
...
> When *integrating* a preserved real subject or an added creature into a new plate (video-to-video), color matching alone reads as "pasted in." Match key direction, environmental bounce, optics/atmospheric haze, and edge grounding together — the full recipe is in `higgsfield-seedance-vfx.md` § Lighting integration.
...
is its sibling, in `higgsfield-facs.md`. Channels say what the body
...
| Soul HEX | Extract color palettes from reference photos for consistent, color-matched visuals across shots |
...
- **`@Image1`** — the platform-side reference syntax used in actual prompts to the
  ...
  `template-*.md` and most sub-skill prompt examples (e.g., `template-06-portrait-character-intro.md`,
  `higgsfield-models-models-deep-reference.md` § The Rule of 12). Pattern in prompts:
  `@Image1 as the character`, `@Image2 as the background`.
- **`<<<image_1>>>`** — the Seedance 2 Skill's internal explicit-reference notation,
  used in user requests to the `seedance-bilingual-json.md` bilingual EN+ZH prompt
...
Paired qualitative + percentage spatial anchors for in-frame placement. `Character A stands in the right third, x-position 70%, y-position 50%, frame occupancy 25%`. The qualitative term gives the film-language hook; the percentage gives the precision target. See `higgsfield-seedance.md` § Frame Coordinate System for the full coordinate system + caveats.
