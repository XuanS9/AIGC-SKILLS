...
For a video project, feed the approved palette, lighting, texture and exclusions into the global visual layer of [shared director planning](narrative-planning.md). Reference collections define style, not a required number or order of shots. Preserve the moodboard creation and application workflows below and carry style changes only at deliberate story transitions.
...
**Example — Y2K Studio:**
```
[MOODBOARD: Summer Portrait Series]
...
- `template-*.md` — Annotated genre templates demonstrating style direction
