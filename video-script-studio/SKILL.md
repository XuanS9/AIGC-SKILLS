...
6. 交付前检查素材职责、人物身份、数量、时序绑定、动作因果、连续状态、连续声场、实际模型限制及适用约束。失败迭代根据当前素材诊断，每轮只改一类变量。
...
- 普通完整视频按八栏目交付：资产与参考锁定、一句话总合成、全局视听与空间、主体行为与节奏规则、分段演出、连续性、结尾状态与交接、关键约束与排除。全局规则写一次，保留分段行动、响应及同时/先后/触发/持续等时序关系。声音先建立全片连续声场；每段只写新增、变化、减弱、消失或被动作遮盖的声音，并明确“其余声音继续继承”，不能把每段的声音清单当成新的音轨。
