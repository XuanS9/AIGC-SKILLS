...
3.0 **对白覆盖核对（合并/拆分/改编片段时强制）：**把确认稿（剧本）里的所有台词逐句列成清单，逐句勾到对应 Clip；每句**内联写在它所属 beat 的动作正文里、在被说出的动作处**，用 `<说话人：台词>` 标注（说话人须是已知角色名，与描述性声音 `<…>` 区分）。无台词片段显式标注“本段无对白（设计）”，不靠“没写”默认。合并批次、精简或改编后若某句找不到落点即视为遗漏，必须补回或说明删改理由；核对可用脚本按“已知说话人”从正文内联 token 中提取台词、与源真值台词总表比对。
...
- **对白内联在动作发生时刻（方案B）：**台词写在【分段演出】对应 beat 的动作正文中、紧接触发它的动作，用 `<说话人：台词>` 标注，使“何时说、在哪个动作说”一目了然；说话人用已知角色名，以便与描述性声音 `<…>` 区分、并可被脚本提取核对。跨批次合并、精简或改编时按上述“对白覆盖核对”逐句回对确认稿并重跑核对脚本，防止台词丢失。无台词片段显式标注“本段无对白（设计）”。
