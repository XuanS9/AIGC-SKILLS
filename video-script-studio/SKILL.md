...
- 完整视频按八栏目交付：资产与参考锁定、一句话总合成、全局视听与空间、主体行为与节奏规则、分段演出、连续性、结尾状态与交接、关键约束与排除。全局规则写一次，分段写行动、响应及同时/先后/触发/持续等时序关系。每个分镜只指定本镜实际需要的声源，跨镜声场一致性由后期处理；默认 NO BGM。**模型、画幅、时长、速度等平台设置写在提示词代码块之外，正文不另加【模型与规格】【规格】等栏目。**
