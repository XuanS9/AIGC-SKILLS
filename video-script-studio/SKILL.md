...
| 触发条件 | 当轮处理 | 仍未解决时 |
|---|---|---|
| 项目没有 `asset-registry.json`，或登记名有而文件未交付 | 从已确认交付建立同一份总表；把无文件条目标为待生成/待绑定。仅凭已登记的调用名不能认定有文件或已绑定；逐项核对 `status`、`file` 和本片段平台绑定。某条资产只提供名称（即使声称“总表有这个名”）而没有 `file`、`status` 和本片段实际绑定时，该条一律按未绑定处理；连看似可沿用的角色标签也不直接写进代码块。未绑定或待补的调用名只列在规划清单；样本提示词正文用总表中的中文名称描述（如阿莉娅、狼）；可复制的提交提示词仅使用当前真实可用的标签，并写明素材前提 | 无法核实名称时询问原名；缺必要文件时交待补素材清单与条件性样本，不声称素材已上传或提示词已可直接生成 |
| 请求的首尾帧、编辑或延伸模式缺少必要输入，或时长与模型当前限制冲突 | 已知只有首帧、却无法读取其画面或既有场景时，仍给一个以“输入首帧”为主体、只描述可通用的自然起势与运镜的约5–8秒条件性视频样本，标明需补首帧/场景后才能按剧情提交，不要求重新上传用户已提供的素材。模式或时长冲突则核对对应模型资料，按内容拆片 | 规格快照过期或仍不能核验时标记待核，不编造画面主体、模型上限、尾帧或生成结果；等待本次视频样本确认才展开全片 |
