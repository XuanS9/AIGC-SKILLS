...
6. 交付前执行下方「输出验证关卡」，检查素材职责、人物身份、数量、时序绑定、动作因果、连续状态、实际模型限制及适用约束；修复文本失败项后再交付，并附实际验证结果。失败迭代根据当前素材诊断，每轮只改一类变量。
...
3. 格式通过后进行语义核对：基础角色不提前带入事故后的湿发、泥污、伤口、破衣或缺鞋（用户明确指定的基础造型除外）；不添加无来源的背带、背包或武器。按正文核对道具何时取得、由谁持有、何时丢失。右侧无头意为取景裁切，保留颈根以下至鞋底，不表现身体断面。名称/文件/status 按资产总表逐项检查；缺表、缺文件或未绑定必须如实记为未检查或待补齐。
...
| 字面检查通过但状态/道具与剧本冲突 | 去掉无依据项或移入对应剧情状态，重做语义核对 | 未定的身份与服装标候选，不虚称已确认 |
...
- 普通完整视频按八栏目交付：资产与参考锁定、一句话总合成、全局视听与空间、主体行为与节奏规则、分段演出、连续性、结尾状态与交接、关键约束与排除。全局规则写一次，保留分段行动、响应及同时/先后/触发/持续等时序关系。
