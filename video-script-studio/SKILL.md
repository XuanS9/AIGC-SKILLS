...
| 触发条件 | 当轮处理 | 仍未解决时 |
|---|---|---|
| 项目没有 `asset-registry.json`，或登记名有而文件未交付 | 从已确认交付建立同一份总表；把无文件条目标为待生成/待绑定，样本中只引用已登记的稳定名称并写明可执行前提 | 无法核实名称时询问原名；缺必要文件时交待补素材清单与条件性样本，不声称素材已上传或提示词已可直接生成 |
| 请求的首尾帧、编辑或延伸模式缺少必要输入，或时长与模型当前限制冲突 | 对照对应模型资料，先确认真正影响模式的一项参数；把长段按内容拆成可承受的生成片段，说明接续状态 | 规格快照过期或仍不能核验时标记待核，不编造模型上限、尾帧或生成结果 |
