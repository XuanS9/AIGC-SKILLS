...
三件事：**页面上怎么写**（格式与动作段落）、**从想法到成稿怎么走**（大纲/处理台本/草稿链/修改）、**改编别人的素材怎么办**。来源：汉森《编剧：步步为营》、沃尔特《剧本》、希克斯《编剧的核心技巧》、菲尔德《电影剧本写作基础》、麦基《故事》、霍克斯特《编剧的十二条法则》、戴蒙德&韦斯曼《好莱坞编剧的生意经》、博克《如何写出好故事》。范例与符号表见 [sw-format-adaptation-reference.md](sw-format-adaptation-reference.md)。行业与买卖见 `sw-industry-business`。
...
> 本节数字来自对多部专业排版剧本（US Letter、Courier 12）的实测与 Final Draft 默认模板，元素级约定与中日行业格式为行业通行做法，**不出自本 skill 的 19 本来源书**（由 issue #2 提议补入）。范例见 [sw-format-adaptation-reference.md](sw-format-adaptation-reference.md) 第十节。
...
**与 `sw-workflow` 的衔接**：阶段 5 的交付物按体例走——好莱坞式交 `.fountain` 文件，中文场号制与日式交纯文本；story-bible 只记文件名与页数。

### 中文影视剧本：场号制

现代中文电影/剧集行业稿的通行结构（各公司细节不同，一稿内统一即可；范例见 sw-format-adaptation-reference.md）：
...
- 企画书（策划书）格式见 sw-format-adaptation-reference.md 第九节，与剧本格式是两种文档。
