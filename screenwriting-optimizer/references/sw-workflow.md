...
4. **没找到且是持续项目**：判断入口路径（见第三节），按 sw-workflow-reference.md 模板新建 `story-bible.md`，只填"项目信息"和"当前阶段"，其余留空。不要一次问用户四个以上的问题；能从用户的话里推断的先填上，标"（待确认）"。
...
- `ozu-screenplay-style`：写反高潮、多主人公、家庭题材、"事件画外反应画内"时，从阶段 2 起作为结构与语气的样板。
...
- `sw-genre-anatomy`（类型解剖）：阶段 0 定主类型与副类型；阶段 1 主控思想对照该类型的人生哲学；阶段 2 结构定稿后过该类型的节拍表，跑混合类型工作单；阶段 3 人物表对照该类型的主角／对手模板；阶段 6 跑类型诊断 12 问。剧集只取它标明的四处剧集结论，反高潮项目不套节拍表。
...
| 将小说 / 舞台剧 / 真实事件改编为影视 | 0 → `sw-format-adaptation` 改编四问 → 1 → 2 …（剧集按下方 S 表推进） | 阶段 1 或 S1 先答"原素材的冲突主要在哪一层面、要重新发明什么"；真人故事先确认授权 |
| 短片 | 1 → 2（用陆军起承转合与八法代替 BS2）→ 3 → 5 | 演示板缩到 10–15 卡；人物≤5；戏核必须先有 |
...
**参照类 skill 何时进入**：`succession-series-writing`（流媒体群像、隐形幕、季形对称、结局工程）从 S2 起作样板；`sw-series-case-studies`（West Wing 四幕、Sopranos pilot、Downton 多线、Fleabag 六集季、Calvisi 八个 pilot 节拍表、坂元裕二、卢熙京）在 S1 找同类 comp、S4 对照节拍。`ozu-screenplay-style` 是**跨阶段表通用参照**（不专属长片）：写反高潮、多主人公、家庭题材、"事件画外反应画内"时，剧集线从 S2 起同样调用，但先过 `ozu-screenplay-style` 二之 1 与二之 14 的容量前件与剧集换算。**长片 skill 的媒介无关部分照用**：对白、场景价值转折、三维人物、前提——不要因为是剧集就跳过它们。
...
不必把对应 skill 全文读一遍。每阶段先读该 skill 的"工作流程"和"诊断清单"两节，需要具体方法再读对应章节，需要范例再读 sw-workflow-reference.md。
...
- **可以并行**的两类工作：
  ...
  2. 大块检索：需要某本 sw-workflow-reference.md 里的具体范例或某部作品的逐幕表时，派 agent 去读并只带回结论，避免主线上下文膨胀。
...
skill 正文只有中文一份，**不为每种语言维护平行译本**。语言这一层按下面三条处理，本 skill 对全部 sw-* skill 生效。

1. **输出语言＝提问语言。** 用户用英语、日语、韩语、法语提问，就用那种语言交付；正文是中文不影响输出语言。用户显式指定语言时以指定为准（"用英文写这版大纲"）。
2. **术语回锚到原词，不自创译名。** 这一行的术语大多原本就是英文（logline、act out、beat sheet、showrunner），中文书里的"计程绳""出幕""剧目管理人"是译文。输出非中文时，凡 [sw-workflow-terms.md](sw-workflow-terms.md) 收录的概念一律使用它的**英文原词**；表里没有的，先判断原词属于哪个语言再锚，不要静默发明。
...
影视交稿后的合作可借鉴以下原则，来源：罗怀臻《当代戏曲编导的觉悟》p0120–0124；此处提取适用于影视的协作思路。

1. 编剧在文本中建立形式，导演通过影像与表演具体呈现；合作前先确认双方对剧本方向的理解。
2. 接受修改意见后，明确由谁改稿，并记录版本与署名约定。
3. 二度创作应准确理解剧本精神；涉及主题、结构、主要人物关系的改动，先协商并写入决策日志。
4. 选择合作者时看能否准确解读、提出建设性意见，以及是否敬业、具备执行与决断能力。
...
- 给 sw-workflow-terms.md 里已有原词的概念现编一个译名。
...
4. 有没有改动已定决策而没记日志？
5. 结束时写了"下一步："吗？
6. 输出语言和提问语言一致吗？术语锚回 sw-workflow-terms.md 的原词了吗？剧本正文的语言没有被对话语言带偏吧？
